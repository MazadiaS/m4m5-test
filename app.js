/* ============================================================
   ...
   ============================================================ */
const WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbxMA1dgR7yPBkBXdVooXb9SbdnqsxhPUGVQyxbdxOBk3lWIxrQKBbocclFcxwKuYv6c/exec";

const STORAGE_KEY = "m4m5_test_state_v1";
...
function renderQuestions() {
  ...
  QUESTIONS.forEach((q) => {
    ...
    div.className = "q";
    div.id = "q" + q.id;
    let html = `<div class="qnum">Question ${q.id} · ${q.topic}</div>
  ...
  wrap.querySelectorAll(".opt").forEach((el) => {
    el.addEventListener("click", () => {
      ...
      // update radio + highlight within this question only
...
async function submitTest() {
  ...
  tb.innerHTML = "";
  Object.keys(r.perTopic).sort().forEach((t) => {
    const p = r.perTopic[t];
    const label = t === "M4" ? "Module 4 (Evaluation)" : "Module 5 (Clustering)";
...
async function sendToSheet(r) {
  ...
  // build a compact answers string: "1:A,2:C,3:-,..."
  ...
  try {
    await fetch(WEBHOOK_URL, {
      method: "POST",
      mode: "no-cors",                       // Apps Script needs no-cors from the browser
      headers: { "Content-Type": "text/plain;charset=utf-8" },
      body: JSON.stringify(payload),
    });
    return true; // no-cors gives an opaque response; we assume success if no throw
  } catch (e) {
    console.error(e);
    return false;
  }
